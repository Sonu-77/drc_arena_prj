...
function StatRow({
  ...
  return (
    ...
    >
      <span className="text-[12px] text-[#F2AEA199]">{label}</span>
      <div className="mt-[2px] h-px w-full border-t border-dashed border-[#F2AEA11F]" />
      <span className={`text-[14px] leading-none ${valueClass}`}>{value}</span>
    </div>
  );
}
...
function TopRankCard({
  ...
  return (
    <div
      className="relative h-[240px] w-full overflow-hidden rounded-[28px] px-5 pb-5 pt-5"
      style={{
        background:
...
<div className=" flex flex-col w-full h-full shrink-0 items-center justify-center rounded-full gap-1">
              ...
              <p className="text-[12px]  text-[#9D7776]">
                RANK
              </p>
        ...
        <div className="flex items-start justify-between gap-3 pb-2">
          <div className="max-w-[56%]">
            ...
            <div className="mt-2" style={{ fontWeight: 200 }}>
              <p className="text-[12px] text-[#F2AEA199]">Acct Value</p>
